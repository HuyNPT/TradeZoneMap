...
namespace TradeMap.Service.CronJob
{
    public class MyCronJob : CronJobService
    {
        ...
        private void HardDeleteBuildingStore()
        {
            ...
            var storeToRemove = _unitOfWork.Repository<Store>().GetAll().Where(x => currentDate >= x.CreateDate && (x.Status == (int)Status.Deleted || x.Status == (int)Status.Reject)).ToList();
